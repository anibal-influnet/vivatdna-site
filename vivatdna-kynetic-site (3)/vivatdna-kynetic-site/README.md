...
- `kynetic-demo.vercel.app` link in the Kynetic section — confirm it's still
  live before sharing.
- The report PDFs and screenshots were originally generated from a real test
  profile ("Anibal Jiménez") in your source materials. The name has since
  been redacted from both downloadable PDFs and all report-gallery
  screenshots — everything public now reads "VivatDNA Test User" instead.
  If you regenerate a new sample report from scratch later, run it through
  the same de-identification pass before publishing it.
